...
function analyzeElement(elt:KumuElement,ctx:Context)
{
	...
	*/
	...
	if(conns.length == 0)
		throw new Error("0:Failed to find 'organization for group' connection for " + elt.label)
	if(conns.length > 1)
	...
	if(!parentOrg) {
		throw new Error("2:Parented working group, but parent not found for elt "+elt.label)
		}

	const pType = parentOrg.fields['SubType']
	if(!pType) {
		throw new Error("3:Parented working group, but parent has no type, elt is "+elt.label+", parent is "+parentOrg.label)
	}

	return "Working Group '"+elt.label+"' is 'w/in a "+parentOrg.fields['SubType']+"' (parent is:"+parentOrg.label+")"
}
